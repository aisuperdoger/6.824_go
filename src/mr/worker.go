...
func DoMapTask(Task ReplyArgs, mapf func(string, string) []KeyValue) bool {
	...
	hashedKva := make(map[int][]KeyValue) // 每个int都对应一个[]KeyValue，即KeyValue切片
	for _, kv := range kva {
		hashed := ihash(kv.Key) % Task.ReduceNum // 每个key都使用hash对应到一个位置，总共有ReduceNum个位置。
		hashedKva[hashed] = append(hashedKva[hashed], kv)
	}

	for i := 0; i < Task.ReduceNum; i++ { // 将对应同一个Reduce的kv写入到同一个文件中
...
func DoReduceTask(Task ReplyArgs, reducef func(string, []string) string) bool {
	...
	*/
	...
	var lines []string
	for i := 0; i < Task.MapNum; i++ { // 很多个Map程序会生成id为TaskID的临时文件，所以使用遍历所有Map程序生成的id为TaskID的临时文件
	...
	*/
	...
	for i < len(kva) {
		j := i + 1
		for j < len(kva) && kva[j].Key == kva[i].Key { // j指向第一个不相等的元素
			j++
		}
		var values []string
		for k := i; k < j; k++ {
...
//
func call(rpcname string, args interface{}, reply interface{}) bool { // 1.8以后，type any = interface{}，所以也可以写成args any
