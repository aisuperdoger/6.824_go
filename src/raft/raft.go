...
type Raft struct {
	...
	peers     []*labrpc.ClientEnd // RPC end points of all peers // ClientEnd包含有一个channel用来向NetWork中发送请求
	...
	currentTerm int
	// votedFor是为了防止出现多个 leader，同个 term 的 follwer 只有一票，一个 term 的 follwer
	// 只能给一个 candidate 投票。票数刷新的时机在于：当前 term 发生转换
	votedFor       int
	...
	nextIndex      []int         // for each server, index of the next log entry to send to that server (initialized to leader last log index + 1) // nextIndex和matchIndex中保存的都是log的下标，log下标是从零开始的
	matchIndex     []int         // for each server, index of highest log entry known to be replicated on server (initialized to -1, increases monotonically)
	applyCh        chan ApplyMsg // applyCh会在shardctrler和shardkv用到，用于通知上层执行已经提交的日志中的命令
...
func (rf *Raft) SendInstallSnapshot(server int) {
	...
	if ok {
		...
		rf.mu.Lock()
		defer rf.mu.Unlock()
		// 1.有可能在进行远程rpc的过程中，其他节点当选了leader。比如本节点网络阻塞，然后又恢复了，此时其他节点可能就当选了leader
		//  其他节点当选leader后，会出现rf.currentTerm != args.Term
...
func (rf *Raft) InstallSnapshot(args *InstallSnapshotArgs, reply *InstallSnapshotReply) {
	...
	defer rf.persist() // 多个defer的执行顺序为后进先出，即栈结构
...
// raft类中没有写定时调用Snapshot()的函数，上层调用raft的类可以自己自定义什么时候保存快照。
func (rf *Raft) Snapshot(index int, snapshot []byte) {
	...
	rf.lastApplied = rf.snapshot.lastIncludedIndex - 1 // 上层必须保证rf.snapshot.lastIncludedIndex - 1之前的日志都已经apply了
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	reply.Term = rf.currentTerm
	if rf.currentTerm > args.Term ||
		(args.Term == rf.currentTerm && rf.votedFor != -1 && rf.votedFor != args.CandidateId) { // 当前节点在currentTerm时，已经给其他candidate投过了票了
		reply.VoteGranted = false
		return
	}

	// 新的任期，重置下投票权
	if rf.currentTerm < args.Term { // term
		rf.currentTerm = args.Term
		rf.SwitchRole(ROLE_Follwer)
	}
		...
		case <-rf.electionTimer.C:
			...
			case ROLE_Follwer:
				...
				*/
				...
				rf.StartElection() // StartElection()中也调用了rf.mu.Lock()，但是它是在协程中调用的，只有锁释放了，才会向下执行。
			}
			rf.mu.Unlock()
		}
	}
...
// 获取最后一个日志在逻辑log中的下标，而返回的是逻辑log的长度，一般后者比前者多一
func (rf *Raft) getLastLogLogicIndex() int {
	return len(rf.log) - 1 + rf.snapshot.lastIncludedIndex // 获取的是逻辑 log（包括已经删除的log）的下标  // lastIncludedIndex下标是从1开始的
}
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	*/
	...
	if realPrevLogIndex >= 0 && rf.log[realPrevLogIndex].Term != args.PrevLogTerm {
		...
		// conflictIndex最终指向ConflictTerm的第一个日志
	...
	// 可能有些日志已经复制到了此节点中，unmatch_idx代表第一个不匹配的日志
	unmatch_idx := -1
	...
	if args.LeaderCommit > rf.commitIndex {
		...
		if commitIndex > rf.getLastLogLogicIndex() { // 不能大于最大的日志索引
...
// 获取真实log的下标，注：下标是从零开始的 // 日志不存在，那么将返回负数
...
func (rf *Raft) SendAppendEntries() {
	...
	for server := range rf.peers {
		...
		go func(server int) {
			...
			// 进行持久化以后的日志，会从rf.log中删除，所以需要计算PrevLogIndex对应的日志下标
			realPrevLogIndex := rf.logicIndexToRealIndex(args.PrevLogIndex)
			...
			if rf.getLastLogLogicIndex() != rf.matchIndex[server] {
				...
				if rf.nextIndex[server] < rf.snapshot.lastIncludedIndex { // lastIncludedIndex从1开始计数，所以如果两者相等时，就代表日志不在快照中
					...
					rf.mu.Unlock()
					go rf.SendInstallSnapshot(server) // 将整个快照发送给follower
					return
				} else {
					startIdx := rf.logicIndexToRealIndex(rf.nextIndex[server]) // log中对应的下标
					...
					if startIdx < rf.getLogRealSize() { // 将follower缺少的日志，全部发送出去
			...
			rf.mu.Lock()
			defer rf.mu.Unlock() // 当前的goroutine执行完毕后，自动执行的函数
			...
			if !reply.Success {
				...
				if reply.ConflictTerm != -1 {
					...
					for i := rf.logicIndexToRealIndex(args.PrevLogIndex) - 1; i >= 0; i-- {
						if rf.log[i].Term == reply.ConflictTerm {
							...
							rf.nextIndex[server] = rf.realIndexToLogicIndex(i) + 1
							break
						}
					}
				}
				...
				for N := rf.getLastLogRealIndex(); N > rf.logicIndexToRealIndex(rf.commitIndex); N-- {
					// if rf.log[N].Term != rf.currentTerm && N==rf.getLastLogRealIndex() {
					// 当前 Leader 只能提交自己任期内的日志，
					// 提交了当前的任期的任意日志时，其实也就把以前其他leader的旧日志也给随带提交了。因为下面代码中rf.setCommitIndex(rf.realIndexToLogicIndex(N))，是将
					// N以前的所有日志给提交了
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	rf.applyCh = applyCh
	for i := range rf.peers {
		rf.matchIndex[i] = -1 // 代表没有一个日志复制到了follower中。
		rf.nextIndex[i] = rf.getLogLogicSize()
	}
