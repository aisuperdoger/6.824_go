...
var ncpu_once sync.Once // 保证在多个 Goroutine 中只有一个 Goroutine 执行Do(func)中的函数
...
func make_config(t *testing.T, n int, unreliable bool, snapshot bool) *config {
	...
	for i := 0; i < cfg.n; i++ {
		...
		cfg.start1(i, applier) // 创建raft节点等
	}
...
// nCommitted(index)：有多少个server已经提交了日志index
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		for si := 0; si < cfg.n; si++ {
			...
			cfg.mu.Unlock()
			if rf != nil {
				// 向leader发送命令。如果当前节点不是leader，那么index1为-1。
				// 如果是leader，那么就会复制日志到log中，并返回日志个数，即索引号
		...
		if index != -1 {
			...
			t1 := time.Now()
			for time.Since(t1).Seconds() < 2 { // 时间间隔小于2，则一直循环
				nd, cmd1 := cfg.nCommitted(index) // 是否有超过expectedServers个节点收到cmd日志
