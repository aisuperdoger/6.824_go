...
func TestInitialElection2A(t *testing.T) {
	servers := 3
	cfg := make_config(t, servers, false, false) // 创建servers个raft服务器，并将它们连接起来
	defer cfg.cleanup()
...
func TestBasicAgree2B(t *testing.T) {
	...
	for index := 1; index < iters+1; index++ {
		...
		xindex := cfg.one(index*100, servers, false) // 向集群发送cmd（index*100）
		if xindex != index { // 相等，代表日志index已经提交成功
